...
def omit_rows(
    dataset: "Dataset", configuration: ConfigTree, to_noise_index: pd.Index
) -> None:
    """
...
def choose_wrong_options(
    ...
    dataset.data.loc[to_noise_index, column_name] = pd.Series(
...
def swap_months_and_days(
    ...
    dataset.data.loc[to_noise_index, column_name] = noised
...
def misreport_ages(
    ...
    dataset.data.loc[to_noise_index, column_name] = new_values
...
def use_nicknames(
    ...
    dataset.data.loc[to_noise_index, column_name] = column
...
def make_phonetic_errors(
    ...
    dataset.data.loc[to_noise_index, column_name] = _corrupt_tokens(
        phonetic_errors,
        dataset.data.loc[to_noise_index, column_name].astype(str),
...
def leave_blanks(
    ...
    :param column_name: String for column that will be noised
    """
    dataset.data.loc[to_noise_idx, column_name] = pd.Series(np.nan, index=to_noise_idx)
    dataset.missingness.loc[to_noise_idx, column_name] = True

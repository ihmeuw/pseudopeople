...
class NoiseType(ABC):
    name: str
    noise_function: Callable[["DatasetData", LayeredConfigTree, pd.Index], None]
...
class RowNoiseType(NoiseType):
    ...
    get_noise_level: Callable[["DatasetData", LayeredConfigTree], float] = lambda _, config: config[
...
class ColumnNoiseType(NoiseType):
    ...
    noise_function: Callable[["DatasetData", LayeredConfigTree, pd.Index, str], None]
